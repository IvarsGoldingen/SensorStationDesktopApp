...
public class StreamMessageHandler implements EventHandler {
	...
	public void onMessage(String event, MessageEvent messageEvent) throws Exception {
		...
		/*
		 * if the the current received event has 2 changed values the json object will
		 * hold within it another json object named data otherwise jsonObject will hold
		 * the single value within it
		 */
	...
	// boolean extractData - true means that the data is to be extracted from the
	// "data" key
	private void extractAndForward(String key, JsonObject jsonObject, boolean extractData) {
		String jsonObjectKey = key;
		if (extractData) {
			jsonObjectKey = "data";
		}
		if (key.equals(SensorStation.CO2_KEY)) {
			retunrResultsToMain(SensorStation.NUM_CO2_KEY, jsonObject.get(jsonObjectKey).getAsInt());
		} else if (key.equals(SensorStation.TVOC_KEY)) {
			retunrResultsToMain(SensorStation.NUM_TVOC_KEY, jsonObject.get(jsonObjectKey).getAsInt());
		} else if (key.equals(SensorStation.T1_KEY)) {
			retunrResultsToMain(SensorStation.NUM_T1_KEY, jsonObject.get(jsonObjectKey).getAsDouble());
		} else if (key.equals(SensorStation.T2_KEY)) {
			retunrResultsToMain(SensorStation.NUM_T2_KEY, jsonObject.get(jsonObjectKey).getAsDouble());
		} else if (key.equals(SensorStation.PRESSURE_KEY)) {
			retunrResultsToMain(SensorStation.NUM_PRESSURE_KEY, jsonObject.get(jsonObjectKey).getAsDouble());
		} else if (key.equals(SensorStation.HUMIDITY_KEY)) {
			retunrResultsToMain(SensorStation.NUM_HUMIDITY_KEY, jsonObject.get(jsonObjectKey).getAsDouble());
		} else if (key.equals(SensorStation.LAST_UPDATE_KEY)) {
			retunrResultsToMain(SensorStation.NUM_LAST_UPDATE_KEY, jsonObject.get(jsonObjectKey).getAsLong());
		} else if (key.equals(SensorStation.DHT_VALID_KEY)) {
			retunrResultsToMain(SensorStation.NUM_DHT_VALID_KEY, jsonObject.get(jsonObjectKey).getAsBoolean());
		} else if (key.equals(SensorStation.SGP_VALID_KEY)) {
			retunrResultsToMain(SensorStation.NUM_SGP_VALID_KEY, jsonObject.get(jsonObjectKey).getAsBoolean());
		}
	}
	...
	public void retunrResultsToMain(int key, int data) {
		if (mainCallback != null) {
			mainCallback.updateStreamData(key, data);
		}
	}
